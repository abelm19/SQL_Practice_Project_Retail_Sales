...
-- which date made most sales(useful for promotions or analyzing highest buying activity
-- Tends to be Q4 and early Q1
select sale_date, max(total_sale) m_t_s, max(profit) as m_p
from retail_sales_profit_included
group by sale_date
...
select sale_date, category, max(total_sale) m_t_s, max(profit) as m_p
from retail_sales_profit_included
group by sale_date, category
order by m_p desc, m_t_s
limit 5;
...
avg(profit) a_p, row_number() over(partition by extract(year from sale_date) order by 
avg(total_sale) desc, avg(profit) desc) as row_num
from retail_sales_profit_included
group by year, month
